...
class Random_Forest():
    ...
    def predict(self, explanatory):
        all_predictions = np.array([pred(explanatory) for pred in self.numpy_preds])

        def mode_along_axis(arr):
            unique, counts = np.unique(arr, return_counts=True)
            return unique[np.argmax(counts)]

        return np.apply_along_axis(mode_along_axis, 0, all_predictions)
